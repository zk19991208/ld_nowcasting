# SimVP 主干网络（Encoder / Mid_Xnet / Decoder），与 SimVP 参考实现一致；类名为 SimVPNet 以免与 Lightning 包装类混淆。
# 用法：from transfer.models.simvp_core import SimVPNet；forward(x) 输入输出均为 (B, T, C, H, W)。
# 运行：由 SimVP_Lit 调用，或单独在 Python 中实例化测试。
