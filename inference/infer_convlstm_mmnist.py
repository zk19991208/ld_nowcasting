# ConvLSTM 在 Moving MNIST 固定测试集（mnist_test_seq.npy）上的离线推理与可视化。
# 流程与 inference/infer_simvp_mmnist.py 一致：读 npy、与 MovingMNIST_Phys 固定集预处理对齐，
# Lightning ckpt 加载后 model(seqs_x) 推理，汇总 MSE/MAE（含逐帧），曲线与样本图写入 output_dir。
#
# 用法（在 transfer 目录下，PyTorch 任务建议 conda activate torch）:
#   python inference/infer_convlstm_mmnist.py --weight_path save/convlstm_mmnist/weights-xxx.ckpt \
...
#     --batch_size 16 --accelerator gpu --gpus 0 --max_samples 10
# 昇腾: --accelerator npu --gpus 1
# 可选: --npy_path；--output_dir（默认 transfer/inference/convlstm_mmnist_infer_out）；
...
_DEFAULT_OUTPUT_DIR = str(_INFERENCE_DIR / "convlstm_mmnist_infer_out")
...
def main() -> None:
    parser = ArgumentParser(description="ConvLSTM Moving MNIST 固定测试集推理")
    ...
    plot_per_frame_curve(
        ...
        "逐帧 MAE（与 ConvLSTM.validation_step 定义一致）",
        os.path.join(output_dir, "per_frame_mae.png"),
    )
    plot_per_frame_curve(
        mse_per_frame,
        "MSE（空间上 sum err^2/B 再对全体 N 条序列平均）",
        "逐帧 MSE（与 ConvLSTM.validation_step 定义一致）",
